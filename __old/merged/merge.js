...
if (process.argv.indexOf('--minify') !== -1) {
	...
	output = Fs.readFileSync(Path.join(process.cwd(), 'bwnode.js')).toString('utf8');
	output = U.minify(output, options);
	Fs.writeFileSync(Path.join(process.cwd(), 'bwnode.js'), license + output.code);
	return;
}
...
Fs.writeFileSync(Path.join(process.cwd(), 'bwnode.js'), output);
